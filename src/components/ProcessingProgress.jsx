...
export default function ProcessingProgress({ file, status, progress }) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-6 animate-fade-in">
      <div className="relative overflow-hidden claude-card bg-white dark:bg-claude-darkCard p-6 shadow-md border-claude-border dark:border-claude-darkBorder">
        <div className="relative z-10">
          {/* Header row */}
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              <div className="p-2 bg-claude-bg dark:bg-claude-darkSecondary rounded-lg">
                <Loader2 className="w-5 h-5 text-claude-accent animate-spin" />
              </div>
              <div>
                <p className="text-base font-bold text-claude-text dark:text-claude-darkText">
                  Processing <span className="text-claude-accent underline decoration-claude-accent/30 decoration-2 underline-offset-4">{file?.name}</span>
                </p>
                <p className="text-xs text-claude-muted dark:text-claude-darkMuted mt-1 uppercase tracking-widest font-bold">
                  {formatFileSize(fileSize)} • {phase}
                </p>
              </div>
            </div>
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border border-claude-border dark:border-claude-darkBorder shadow-sm ${speed.color}`}>
              <SpeedIcon className="w-3.5 h-3.5" />
              <span className="text-xs font-bold tracking-tight">{speed.label}</span>
            </div>
          </div>

          {/* Progress bar */}
          <div className="relative h-3 bg-claude-bg dark:bg-claude-darkSecondary rounded-full overflow-hidden shadow-inner border border-claude-border dark:border-claude-darkBorder">
            <div
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-claude-muted dark:text-claude-darkMuted bg-claude-bg dark:bg-claude-darkSecondary px-2 py-1 rounded-md">
                {elapsed > 0 && `${Math.round(elapsed)}s ELAPSED`}
              </span>
              {totalSteps > 1 && currentStep > 0 && (
                <span className="text-xs font-bold text-claude-accent bg-claude-accent/5 dark:bg-claude-accent/10 px-2 py-1 rounded-md">
                  STEP {currentStep} / {totalSteps}
                </span>
              )}
            </div>
            <p className="text-lg font-serif font-bold text-claude-text dark:text-claude-darkText">
          ...
          {fileSize > 5 * 1024 * 1024 && (
            <div className="mt-6 p-4 bg-claude-bg dark:bg-claude-darkSecondary rounded-xl border border-claude-border dark:border-claude-darkBorder flex gap-3">
